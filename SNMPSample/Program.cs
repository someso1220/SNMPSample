...
partial class Program
{
    ...
    {
        // まずは標準の Type.GetType() で探す
        var type = Type.GetType(typeName);
        if (type != null)
        {
            return type; // 見つかったらそのまま返す
        }

        // 現在ロードされている全アセンブリを走査して探す
        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
        {
            type = assembly.GetType(typeName);
            if (type != null)
            {
                return type; // 見つかったら返す
            }
        }

        // どこにも見つからなければ null を返す
        return null;
    }
    ...
    {
        string folderPath = @"D:\Log" + "\\" + string.Format(@"{0:yyyyMMdd}", DateTime.Now);
        ...
        try
        {
            ...
            using (StreamWriter writer = new StreamWriter(folderPath + "\\" + fileName, append: true))
            {
        ...
        {
            Console.WriteLine("ログの出力に失敗しました\n" +  ex.ToString());
        }
